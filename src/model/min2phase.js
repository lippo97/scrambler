...
const min2phase = (function () {
	...
	const ud2std = [Ux1, Ux2, Ux3, Rx2, Fx2, Dx1, Dx2, Dx3, Lx2, Bx2, Rx1, Rx3, Fx1, Fx3, Lx1, Lx3, Bx1, Bx3];
	const std2ud = [];
	const ckmv2bit = [];
	const urfMove = [
	...
	{ // Init util
		...
		for (var i = 0; i < 10; i++) {
			const ix = Math.trunc(ud2std[i] / 3);
			ckmv2bit[i] = 0;
			for (var j = 0; j < 10; j++) {
				const jx = Math.trunc(ud2std[j] / 3);
	...
	function setValue(value0, value, isEdge) {
		return isEdge ? (value << 1 | value0 & 1) : (value | value0 & 0xF8);
	}

	function getValue(value0, isEdge) {
	...
	function setNPerm(array, idx, n, isEdge) {
		n--;
		let value = 0x76543210;
		for (let i = 0; i < n; ++i) {
			const p = fact[n - i];
			let v = Math.trunc(idx / p);
			idx %= p;
			v <<= 2;
			array[i] = setValue(array[i], value >> v & 0xF, isEdge);
			const m = (1 << v) - 1;
			value = (value & m) + (value >> 4 & ~m);
		}

		array[n] = setValue(array[n], value & 0xF, isEdge);
	}

	function getNPerm(array, n, isEdge) {
		let idx = 0;
		let value = 0x76543210;
		for (let i = 0; i < n - 1; ++i) {
			const v = getValue(array[i], isEdge) << 2;
			idx = (n - i) * idx + (value >> v & 0xF);
			value -= 0x11111110 << v;
		}
	...
	function setNPermFull(array, idx, n, isEdge) {
		array[n - 1] = setValue(array[n - 1], 0, isEdge);
		for (let i = n - 2; i >= 0; --i) {
			array[i] = setValue(array[i], idx % (n - i), isEdge);
			idx = Math.trunc(idx / (n - i));
			for (let j = i + 1; j < n; ++j) {
				if (getValue(array[j], isEdge) >= getValue(array[i], isEdge)) {
					array[j] = setValue(array[j], getValue(array[j], isEdge) + 1, isEdge);
				}
			}
		}
	}

	function getNPermFull(array, n, isEdge) {
		let idx = 0;
		for (let i = 0; i < n; ++i) {
			idx *= n - i;
			for (let j = i + 1; j < n; ++j) {
	...
	function getComb(array, mask, isEdge) {
		const end = array.length - 1;
		let idxC = 0;
		let r = 4;
		for (let i = end; i >= 0; i--) {
			const perm = getValue(array[i], isEdge);
			if ((perm & 0xC) == mask) {
				idxC += Cnk[i][r--];
			}
		}
	...
	function setComb(array, idxC, mask, isEdge) {
		const end = array.length - 1;
		let r = 4;
		let fill = end;
		for (let i = end; i >= 0; i--) {
			if (idxC >= Cnk[i][r]) {
				idxC -= Cnk[i][r--];
				array[i] = setValue(array[i], r | mask, isEdge);
			} else {
				if ((fill & 0xC) == mask) {
					fill -= 4;
				}
	...
	CubieCube.prototype.setFlip = function (idx) {
		...
		for (let i = 10; i >= 0; i--, idx >>= 1) {
			parity ^= (value = idx & 1);
			this.ea[i] = this.ea[i] & 0xFE | value;
		}
	...
	CubieCube.prototype.setTwist = function (idx) {
		let twst = 15;
		let value;
		for (let i = 6; i >= 0; i--, idx = Math.trunc(idx / 3)) {
			twst -= (value = idx % 3);
			this.ca[i] = this.ca[i] & 0x7 | value << 3;
		}
	...
	CubieCube.prototype.toFaceCube = function (cFacelet, eFacelet) {
		...
		for (let c = 0; c < 8; c++) {
			...
			for (var n = 0; n < 3; n++) {
				f[cFacelet[c][(n + ori) % 3]] = ts[Math.trunc(cFacelet[j][n] / 9)];
			}
		}
		...
		for (let e = 0; e < 12; e++) {
			...
			for (var n = 0; n < 2; n++) {
				f[eFacelet[e][(n + ori) % 2]] = ts[Math.trunc(eFacelet[j][n] / 9)];
			}
		}
	...
	CubieCube.prototype.fromFacelet = function (facelet, cFacelet, eFacelet) {
		...
		let col1; let col2; var i; let j; let ori;
		for (i = 0; i < 8; ++i) {
			for (ori = 0; ori < 3; ++ori) {
			...
			for (j = 0; j < 8; ++j) {
				if (col1 == Math.trunc(cFacelet[j][1] / 9) && col2 == Math.trunc(cFacelet[j][2] / 9)) {
					this.ca[i] = j | ori % 3 << 3;
					break;
				}
			}
		...
		for (i = 0; i < 12; ++i) {
			for (j = 0; j < 12; ++j) {
				if (f[eFacelet[i][0]] == Math.trunc(eFacelet[j][0] / 9) && f[eFacelet[i][1]] == Math.trunc(eFacelet[j][1] / 9)) {
					this.ea[i] = j << 1;
					break;
				}

				if (f[eFacelet[i][0]] == Math.trunc(eFacelet[j][1] / 9) && f[eFacelet[i][1]] == Math.trunc(eFacelet[j][0] / 9)) {
					this.ea[i] = j << 1 | 1;
					break;
				}
			}
	...
	Search.prototype.initSearch = function () {
		...
		for (let i = 0; i < 6; i++) {
			...
			if (i % 3 == 2) {
				const temporary = new CubieCube().invFrom(this.cc);
				this.cc.init(temporary.ca, temporary.ea);
			}
		}
	};
	...
	Search.prototype.phase1PreMoves = function (maxl, lm, cc) {
		...
		for (let m = 0; m < 18; m++) {
			...
			const returnValue = this.phase1PreMoves(maxl - 1, m, this.preMoveCubes[maxl]);
			if (returnValue == 0) {
				return 0;
			}
		}
	...
	Search.prototype.initPhase2Pre = function () {
		...
		let returnValue = this.initPhase2(this.phase1Cubie[this.depth1]);
		if (returnValue == 0 || this.preMoveLen == 0 || returnValue == 2) {
			return returnValue;
		}

		const m = Math.trunc(this.preMoves[this.preMoveLen - 1] / 3) * 3 + 1;
		...
		returnValue = this.initPhase2(this.phase1Cubie[this.depth1 + 1]);
		this.preMoves[this.preMoveLen - 1] += 2 - this.preMoves[this.preMoveLen - 1] % 3 * 2;
		return returnValue;
	};
	...
	Search.prototype.initPhase2 = function (phase2Cubie) {
		...
		const p2edgei = getPermSymInv(p2edge, p2esym, false);
		const p2corni = getPermSymInv(p2corn, p2csym, true);
		const prun = Math.max(
		...
		for (depth2 = maxDep2 - 1; depth2 >= prun; depth2--) {
			const returnValue = this.phase2(p2edge, p2esym, p2corn, p2csym, p2mid, depth2, this.depth1, 10);
			if (returnValue < 0) {
				break;
			}
	...
	Search.prototype.phase1 = function (node, maxl, lm) {
		if (node.prun == 0 && maxl < 5) {
			if (this.allowShorter || maxl == 0) {
				this.depth1 -= maxl;
				var returnValue = this.initPhase2Pre();
				this.depth1 += maxl;
				return returnValue;
			}
		...
		for (let axis = 0; axis < 18; axis += 3) {
			...
			for (let power = 0; power < 3; power++) {
				...
				var returnValue = this.phase1(this.nodeUD[maxl], maxl - 1, axis);
				if (returnValue == 0) {
					return 0;
				}
	...
	Search.prototype.appendSolMove = function (curMove) {
		...
		const axisCur = Math.trunc(curMove / 3);
		const axisLast = Math.trunc(this.moveSol[this.moveSol.length - 1] / 3);
		if (axisCur == axisLast) {
		...
		if (this.moveSol.length > 1 &&
			...
			axisCur == Math.trunc(this.moveSol[this.moveSol.length - 2] / 3)) {
	...
	Search.prototype.phase2 = function (edge, esym, corn, csym, mid, maxl, depth, lm) {
		...
		for (let m = 0; m < 10; m++) {
			...
			let prun = getPruningMax(EPermCCombPPrunMax, EPermCCombPPrun,
			...
			const returnValue = this.phase2(edgex, esymx, cornx, csymx, midx, maxl - 1, depth + 1, m);
			if (returnValue >= 0) {
	...
	Search.prototype.solutionToString = function () {
		...
		if (urf < 3) {
			for (var s = 0; s < this.moveSol.length; ++s) {
				sb += move2string[urfMove[urf][this.moveSol[s]]] + ' ';
			}
		} else {
		...
		{ // Init coord tables
			...
			for (var i = 0; i < N_COMB; i++) {
				...
				for (var j = 0; j < N_MOVES2; j++) {
					...
					CCombPMove[i][j] = d.getCComb() + 70 * ((P2_PARITY_MOVE >> j & 1) ^ Math.trunc(i / 70));
				}

				for (var j = 0; j < 16; j++) {
					CubieCube.CornConjugate(c, SymMultInv[0][j], d);
					CCombPConj[i][j] = d.getCComb() + 70 * Math.trunc(i / 70);
				}
			}
		}
	}
	...
	function initRawSymPrun(PrunTable, N_RAW, N_SYM, RawMove, RawConj, SymMove, SymState, PrunFlag) {
		...
		while (depth < SEARCH_DEPTH) {
			...
			const selArrayMask = select * 0x11111111;
			const check = inv ? depth : 0xF;
			depth++;
			InitPrunProgress++;
			const xorValue = depth ^ 0xF;
			let done = 0;
			let value = 0;
			for (var i = 0; i < N_SIZE; i++, value >>= 4) {
				if ((i & 7) == 0) {
					value = PrunTable[i >> 3];
					if (!hasZero(value ^ selArrayMask)) {
						i += 7;
						continue;
					}
				}
				...
				for (let m = 0; m < N_MOVES; m++) {
					...
					if (inv) {
						setPruning(PrunTable, i, xorValue);
						break;
					}
					...
					for (let j = 1, symState = SymState[symx];
						...
						let idxx = symx * N_RAW;
						idxx += ISTFP ? FlipS2RF[FlipR2S[rawx] ^ j] : RawConj[rawx][j ^ (SYM_E2C_MAGIC >> (j << 1) & 3)];
						if (getPruning(PrunTable, idxx) == check) {
							setPruning(PrunTable, idxx, xorValue);
							done++;
						}
					}
				}
	...
	function randomCube() {
		let ep; let cp;
		const eo = Math.trunc(Math.random() * 2048);
		const co = Math.trunc(Math.random() * 2187);
		do {
			ep = Math.trunc(Math.random() * fact[12]);
			cp = Math.trunc(Math.random() * fact[8]);
